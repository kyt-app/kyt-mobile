...
Future<String> getTestDetails(String testName, String email) async {
  final testDetailsEndpoint = Uri.parse('https://kyt-api.azurewebsites.net/tests/testdetails?email=$email&testName=$testName');
            ...
            child: Column(
              ...
              children: <Widget>[
                Center(
                  child: Ink(
                    decoration: ShapeDecoration(
                      color: details['status'] == 'valid' ? MyColors.green : Colors.red,
                      shape: CircleBorder()
                    ),
                    child: Icon(
                      details['status'] == 'valid' ? Icons.check : Icons.clear,
                      color: Colors.white,
                      size: 120.0
                    )
                  )
                ...
                Center(
                  child: Text(
                    details['testName'],
                    style: Theme.of(context).textTheme.headline3.copyWith(color: MyColors.darkPrimary)
                  ),
                ),
                MySpaces.vMediumGapInBetween,
                Text(
                  'Date issued',
                  style: Theme.of(context).textTheme.headline5.copyWith(color: MyColors.darkPrimary),
                ),
                MySpaces.vGapInBetween,
                VerifyCard(
                  text: DateFormat.yMMMMEEEEd('en_US').format(DateTime.parse(details['timestamp'])),
                ),
                MySpaces.vMediumGapInBetween,
                Text(
                  'Valid until',
